...
def expect_blocks_of_day(day_number)
  ...
  expect(schedule.periods.map {|p| p.block})
end
...
describe 'Obtaining time-based schedules for days' do
  ...
  it 'should tell period at time' do
    expect(Schedule.of_day(StandardDay.new(2)).period_index_at_time(time(12, 12))).to be 3
    expect(Schedule.of_day(StandardDay.new(6)).period_index_at_time(time(7, 44))).to be 0
    expect(Schedule.of_day(StandardDay.new(8)).period_at_time(time(16, 22))).to be_nil
  end
end